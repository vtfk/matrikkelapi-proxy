...
module.exports = class WSDLClient {
  ...
  static findXsiType (namespace, method, itemType) {
    ...
    console.log('Schema is found: ' + schema);
    // Read the file
    ...
    // Find all namespaces
    let namespaces = [];
    let _namespaces = xmlQuery(parsed).find('xs:schema').attr();
    if(_namespaces) {
      for(let ns in _namespaces) {
    ...
    xmlQuery(foundDefinition).find('xs:element').each((e) => {
      ...
      if(e.name == itemType) {
        ...
        console.log('Checking type against another file');
        let nsid = e.attributes.type.split(':')[0];
        let nextType = e.attributes.type.split(':')[1];
        console.log('Namespace id: ' + nsid);
        let matchNs = namespaces.find((ns) => ns.id == nsid);
        console.log('Matched namespace: ' + matchNs.namespace);
        console.log('Matched schema: ' + matchNs.schemaLocation);

        // TODO: MAKE THIS RECURSIVE
        // Load the file
        let file = this.readFile(matchNs.schemaLocation);
        // Parse the file
        let subParsed = XmlReader.parseSync(file);
        // Find the new type
