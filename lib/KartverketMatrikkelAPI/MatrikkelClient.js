  ...
  async makeRequest (req, body) {
    ...
    if (req.query.flatten) {
      ...
      for (let i = 0; i < responseBody.length; i++) {
        ...
        let firstProperty = response[Object.keys(response)[0]];

        // If the response has been resolved the two first ifs should return correctlly
        if (response.item) { items.push(...response.item) }
        else if (response.items) { items.push(...response.items) }
