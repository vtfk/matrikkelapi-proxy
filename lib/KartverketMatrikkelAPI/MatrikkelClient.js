      ...
      for (let j = 0; j < _itemGroups.length; j++) {
        ...
        for (let ii = 0; ii < _items.length; ii++) {
          ...
          if(!itemXSINamespace) {
            ...
            console.log('Attempting to get WSDL definition')
            console.log('Namespace: ' + namespaceString);
            console.log('Method: ' + response.type);
            console.log('Item: ' + itemKeyType);
            let result = WSDLClient.findXsiType(namespaceString, response.type, itemKeyType);
