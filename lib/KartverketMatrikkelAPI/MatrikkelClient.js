...
class MatrikkelClient {
  ...
  */
  ...
  getItemsFromJSONParsedXMLResponse(data) {
    ...
    let responses = [];   // Array that contains all the responses

    // Get response body if exist
    let body = _.get(updatedData, 'Envelope.Body');
    ...
    for(let bi = 0; bi < body.length; bi++) {
      ...
      let responseItems = response[0];
      if(responseItems['$']) {
        let tmpNamespaces = responseItems['$'];
        for(let obj in tmpNamespaces) {
      ...
      let responseValues = responseItems[Object.keys(responseItems)[1]];
      if(responseValues) {
        for(let rvi = 0; rvi < responseValues.length; rvi++) {
          let valueGroup = responseValues[rvi];
          let valueGroupKey = Object.keys(valueGroup)[0];
          ...
          for(let i = 0; i < valueGroup.length; i++) {
            ...
            // Resolve namespaces
            let matchedNamespace = responseObject.namespaces.filter((ns) => ns.id == itemObject.nsid)
  ...
  async makeRequest (req, body) {
    ...
    try {
      ...
      // responseBody = this.getItemsFromJSONParsedXMLResponse(responseBody);
      return responseBody;
    } catch(err) {
