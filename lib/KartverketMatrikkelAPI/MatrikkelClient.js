...
class MatrikkelClient {
  ...
  */
  ...
  lightlyResolveRequest (rawXML, parsedJSON) {
    // Input validation
    ...
    function lighlyResolve (item, key, parent) {
      // Recursively check every child if array
      if (Array.isArray(item)) {
        item.forEach((i) => { lighlyResolve(i, key, item) })
      }

      // Recursively check every property if present
      if (typeof item === 'object') {
        Object.keys(item).forEach((key) => {
          lighlyResolve(item[key], key, item);
        })
      }
      ...
      if (item.$ && item.$['xsi:type'] && item.$['xsi:type'] && Object.keys(item.$).length <= 2) {
        try {
          delete parent[key].$;
        } catch {}
      }
    ...
    copy.forEach((item, i) => {
      copy[i] = lighlyResolve(item, undefined, undefined);
    })
  ...
  async makeRequest (req, body) {
    ...
    if (req.query.resolve) {
      ...
      cleanJSON = this.lightlyResolveRequest(responseBody, cleanJSON);
      responseBody = cleanJSON;
    }
    ...
    if (req.query.flatten) {
      ...
      items.forEach((item) => {
        // Doing it this way might not be a good idéa, we will see..
        if (Array.isArray(item)) {
