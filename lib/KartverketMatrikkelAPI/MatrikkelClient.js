...
/**
 * Retreives company information for Brreg
 * @param {String} orgnr The organization number of the company
 * @returns
 */
async function getCompanyFromBrreg (orgnr) {
  ...
  try {
    ...
    try {
      const branchResponse = await axios.get(`https://data.brreg.no/enhetsregisteret/api/underenheter/${orgnr}`);
...
/**
 * Retreives a persons information from DSF (Det Sentrale Folkeregister)
 * @param {String} personnr Personnumber
 * @returns
 */
async function getPersonFromDsf (personnr) {
  // Create a JWT token for contacting VTFK DSF API
  const payload = {
    system: pkg.name,
    version: pkg.version
  }
  ...
  const token = jwt.sign(payload, config.VTFK_JWTAPI_SECRET, options)

  // Prepare the request
  const request = {
    method: 'POST',
    url: config.VTFK_DSFAPI_ENDPOINT,
    headers: {
      Authorization: `${token}`
    },
    data: {
 ...
 * If the response contains persons DSF will be contacted
...
class MatrikkelClient {
  constructor (username, password, endpoint, baseurl) {
    this.baseurl = baseurl || config.MATRIKKELAPI_BASEURL;
    this.username = username || config.MATRIKKELAPI_USERNAME;
    this.password = password || config.MATRIKKELAPI_USERNAME;
  ...
  */
  ...
  // Removes namespace information in front of the tag
  ...
  /*
    Matrikkel spesifikt
    resolveRequest and lightlyResolveRequest could probably be merged to one function
    They were kept sepparate due to time constraints
  */
